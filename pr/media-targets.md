# Media Targets — contactos y proceso

Nota: los nombres de editores en medios cambian con frecuencia. Verifica cada contacto en la página oficial del medio antes de enviar (los enlaces de "fuente" quedan abajo de cada uno). Si un correo rebota, usa el formulario de contacto general como respaldo.

## Prensa escrita / digital

### Forbes México
- Contacto editorial: `aromero@forbes.com.mx` (Editor Forbes Life)
- Directorio actualizado: https://forbes.com.mx/tag/contacto/
- Pitch a usar: `pitch-emails.md` #1

### Entrepreneur en Español
- Editora en jefe: **Martha Elena Violante** — `mviolante@g21.com.mx`
- Asunto sugerido: "Interesado en colaborar con temas de autoconocimiento y liderazgo"
- Nota importante: aceptan **3 publicaciones de prueba** antes de decidir si sigues como colaborador — empieza con tu mejor artículo
- Requisitos: textos bien escritos, sin errores; toda cita/estadística debe enlazar a la fuente original; no se permite mencionar empresas con relación comercial sin declararlo
- Pitch a usar: `pitch-emails.md` #2

### Expansión
- Director Editorial: **Gonzalo Soto** — `gonzalo.soto@grupoexpansion.com`
- Jefe de Redacción: `mario.verdusco@grupoexpansion.com` (verificar nombre/cargo actual antes de enviar, la fuente estaba desactualizada)
- Contacto general: servicioaclientes@expansion.com.mx / (55) 9177 4342
- Pitch a usar: `pitch-emails.md` #3

### Infobae
- Redacción / envío de notas: `redaccion@infobae.com`
- Contacto general: `contacto@infobae.com`
- Sección de Opinión (para ver el formato que usan): https://www.infobae.com/opinion/
- Pitch: adapta el #1 (Forbes) o el #3 (Expansión), Infobae tiene un tono más noticioso — lidera con el ángulo humano (la historia de tu padre) más que con el ángulo puramente de negocio

## Podcasts LATAM de desarrollo personal / autoconocimiento

Estos no tienen un email de contacto único y público — la forma más efectiva de llegar a ellos es por Instagram (DM o el email que casi siempre ponen en su bio) o por el email que dejan en las notas del episodio en Spotify/Apple Podcasts. Investiga cada uno antes de escribir:

- **Se Regalan Dudas** (Lety Sahagún y Ashley Frangie) — de los más grandes en México/Colombia/España, enfoque en amor propio y crecimiento personal
- **Cambio de Mentalidad** (Diego Barrazas) — entrevistas con personas que "están cambiando el mundo", formato compatible con tu historia
- **Las Claves de Sol** (Sol Aguirre) — enfocado específicamente en autoconocimiento y desarrollo personal
- **Entiende tu Mente** — psicología aplicada, episodios cortos
- **Conversaciones Inspiradoras** (Jesús) — historias de superación y autoconocimiento

Proceso sugerido: busca cada uno en Instagram, revisa sus últimos 5-10 episodios para confirmar que el formato encaja contigo, y usa el pitch #4 (`pitch-emails.md`) adaptado al nombre del conductor/a.

## Podcasts específicamente colombianos (o con host colombiano)

Estos tres tienen mayor probabilidad real de responderte, porque el formato de entrevistas con emprendedores/expertos/historias de reinvención encaja directamente con tu historia (ejecutivo corporativo → autor → método propio):

### Roca Project (Carlos Roca)
- Conversaciones a fondo con expertos, personas influyentes y también gente anónima con historias de impacto — el formato es casi calcado a tu pitch
- Contacto: `info@rocaproject.com`
- Spotify: https://open.spotify.com/show/5XWpDtVEGxBhRJ4h9p2nBd

### Aldea Global con Kike Posada
- Kike Posada nació en Colombia (fue pionero del rock en español en radio comercial de EE.UU. con el programa "Boom" en Miami). Su podcast actual es de crecimiento personal, entrevistas, coaching y contenido holístico — encaja bien con el ángulo de autoconocimiento, menos con el ángulo puramente de negocio
- Contacto: `kikeboom@gmail.com`
- Apple Podcasts: https://podcasts.apple.com/us/podcast/aldea-global-con-kike-posada/id1440528714

### Yo, Camilo (Camilo Barberi)
- Recibe líderes, emprendedores, expertos y "soñadores" para hablar sin filtro de éxito, fracaso, resiliencia y decisiones que marcan el camino — tu historia (dejar un cargo corporativo estable) encaja directo con su formato
- No encontré email público; contáctalo por Instagram o por el perfil de creador en Spotify: https://creators.spotify.com/pod/profile/czuluagarestrepo2/

**Nota:** no incluí "Creadores" (creadores.co) que apareció en una búsqueda anterior porque su host, Marcelo Zegarra, es peruano, no colombiano — lo descarté para no darte un dato incorrecto.

Empieza por **Roca Project** — es el que mejor encaja en formato y el que tiene contacto directo confirmado.

## Medios en inglés (para cuando esté lista *The Self-Powered Life*)

- **Entrepreneur** (versión internacional) y **Inc.** tienen programas de colaboradores similares al de Entrepreneur en Español — busca "become a contributor" en cada sitio
- **Fast Company** recibe pitches directamente a los editores de sección (Leadership/Work Life) — mejor esperar a tener más tracción en inglés antes de este

## Seguimiento de envíos

| Fecha | Destinatario | Canal | Propuesta | Estado |
|---|---|---|---|---|
| 2026-08-13 | Roca Project (Carlos Roca) | Email | Ser invitado en su podcast | Enviado |
| 2026-08-13 | Juan Pablo Raba ("Los Hombres Sí Lloran") | Instagram DM | Ser invitado en su podcast | Enviado |
| 2026-08-13 | Johanna Gómez | LinkedIn (mensaje directo, ya conectados) | Colaboración — LinkedIn Live o entrevista cruzada de blog | Enviado |

Actualiza esta tabla cada vez que envíes uno nuevo o recibas respuesta — así sabes en qué punto está cada conversación sin tener que revisar tu correo.

## Cadencia recomendada

1-2 pitches por semana, nunca todos el mismo día. Empieza por Entrepreneur en Español (proceso más claro y accesible), sigue con un podcast, y deja Forbes/Expansión para cuando tengas al menos una publicación de referencia que puedas mencionar en el siguiente pitch ("como lo publiqué recientemente en...").

---

**Fuentes consultadas:**
- [forbes.com.mx/tag/contacto](https://forbes.com.mx/tag/contacto/)
- [entrepreneur.com/page/323377](https://www.entrepreneur.com/page/323377) — guía para colaboradores
- [expansion.mx/directorio](https://expansion.mx/directorio)
- [mediakit.infobae.com/contacto](https://mediakit.infobae.com/contacto/)
- [infobae.com/contacto](https://www.infobae.com/contacto/)
